...
def warm_up_models(group_size_min, group_size_max):
    with httpclient.InferenceServerClient(triton_http_endpoint) as client:
        ...
        # collect all models loaded and feature dimensions
        models = client.get_model_repository_index()
        for model in models:
            model_name = model['name']
            # skip the warmup_helper itself
            if model_name == warmup_helper:
                continue
            ...
            for input in inputs:
                if input["name"] == 'x':
                    feature_dim = input['dims'][1]
                    feature_dims.append(feature_dim)
            if feature_dim is None:
                raise Exception(f"No feature dim found for model {model_name}")
        ...
        outputs = [httpclient.InferRequestedOutput("REQUEST_COUNT")]

        # start the warmup
        response = client.infer(warmup_helper, inputs, request_id=str(1), outputs=outputs)
        # get the warmup result
        output_data = response.as_numpy("REQUEST_COUNT")

        # check if enough request has been sent
        assert output_data[0] == len(model_names) * (group_size_max - group_size_min + 1)
        print(f"number of warmup request sent: {output_data[0]}")
        return model_names
